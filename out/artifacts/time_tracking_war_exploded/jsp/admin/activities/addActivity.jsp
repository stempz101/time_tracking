...
<section class="container">
    <div class="row">
        <div class="col-12 text-center"><h3>Add activity</h3></div>
        <div class="col-2"></div>
        <form action="<%= request.getContextPath() + "/a/add-act" %>" class="col-8 border rounded mt-2"
              method="post"
              enctype="multipart/form-data">
            <div class="form-group mx-4">
                <label for="activityName" class="col-form-label-lg">Activity Name <p class="d-inline-block mb-0"
                                                                                     style="color: red">*</p></label>
                <input type="text" class="form-control" id="activityName" name="activityName"
                       value="<c:out value="${sessionScope.activity.name}" />" placeholder="Name"
                       pattern="[а-щА-ЩЬьЮюЯяЇїІіЄєҐґ'\w\s]+" required>
                <c:if test="${sessionScope.activityNameError != null}">
                    <div class="alert alert-danger mt-3 py-1">${sessionScope.activityNameError}</div>
                </c:if>
            </div>
            <div class="form-group mx-4">
                <label for="activityCategory" class="col-form-label-lg">Category</label>
                ...
                <c:if test="${sessionScope.activityCategoriesError != null}">
                    <div class="alert alert-danger mt-3 py-1">${sessionScope.activityCategoriesError}</div>
                </c:if>
            </div>
            <div class="form-group mx-4">
                <label for="activityDescription" class="col-form-label-lg">Description <p class="d-inline-block mb-0"
                                                                                          style="color: red">*</p>
                </label>
                <textarea class="form-control" id="activityDescription" name="activityDescription" rows="8"
                          placeholder="Lorem ipsum ..." required>${sessionScope.activity.description}</textarea>
                <c:if test="${sessionScope.activityDescriptionError != null}">
                    <div class="alert alert-danger mt-3 py-1">${sessionScope.activityDescriptionError}</div>
                </c:if>
            </div>
            <div class="form-group mx-4">
                <label for="activityImage" class="col-form-label-lg">Preview</label>
            ...
            <div class="row form-group mx-4 my-3">
                <button class="btn btn-lg btn-primary" type="submit" name="sendMe" value="1">Create</button>
            </div>
...
<%
    ...
    session.removeAttribute("activityNameError");
    session.removeAttribute("activityCategoriesError");
    session.removeAttribute("activityDescriptionError");
